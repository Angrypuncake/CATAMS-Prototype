...
export default function SwapReview({
  ...
  useEffect(() => {
    if (!data || data.requestType !== "swap") return;
  ...
  const resolvedSuggestedTutorId: number | null = (() => {
    ...
    if (
      suggestedTutor &&
      typeof (suggestedTutor as unknown as Record<string, unknown>)[
        "user_id"
      ] === "number"
    ) {
      return (suggestedTutor as unknown as Record<string, number>)["user_id"];
    }
    if (
      suggestedTutor &&
      typeof (suggestedTutor as unknown as Record<string, unknown>)[
      ...
      return (suggestedTutor as unknown as Record<string, number>)["tutor_id"];
    }
    if (
      suggestedTutor &&
      typeof (suggestedTutor as unknown as Record<string, unknown>)["id"] ===
        "number"
    ) {
      return (suggestedTutor as unknown as Record<string, number>)["id"];
    }
    return null;
  })();
      ...
      return taForwardWithDetails(
        ...
        11,
        {
          ...
          details: {
            ...
            suggested_alloc_id: selectedAllocation
              ? Number(selectedAllocation.id)
              : null,
          },
        },
        undefined, // reason
